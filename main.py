        ...
        if ('first_name' in player):
            ...
            if ('history' in json_player_data):
                ...
                for i,p in enumerate(points):
                    ...
                    player_points.append(p['total_points'])
...
def test_output():
    ...
    for player in player_id:
        if count == 5:
            print(player)
        count = count +1
    X_train,Y_train,Mappings = convert_func(player_id)
    X_pred = next_game_averages(X_train)
    Y_pred = linear_reg(X_train,Y_train, X_pred)
    players_new = turn_into_dic(Mappings,Y_pred)

    player_name = []
    for val in players_new.keys():
        player_name.append(val[1])
    xvalue = player_name
    yvalue = list(players_new.values())
...
def master_run_func(start,duration,data_set):
    ...
    X_train, Y_train = start_and_duaration(X,Y,start,duration)
    X_pred = next_game_averages(X_train)
    predicted = linear_reg(X_train,Y_train, X_pred)
    actual = []
    accuracy = []
    for i, Y_value in enumerate(Y):
    ...
    for i,val in enumerate(mappings):
        combined_list.append((val[1],predicted[i],actual[i],accuracy[i]))
...
data_set=load_from_api()
pred,actual,accuracy,top10p,top10a = master_run_func(0,24,data_set)
print(top10p)
print(top10a)