...
for i in range(len(x_train)):
    elem2 = x_train[i]-convolve2d(x_train[i].reshape(28,28),np.array([[1,2,1],[2,4,2],[1,2,1]])/16, mode='same', boundary='fill', fillvalue=0)
    x_train[i] = (elem2-np.amin(elem2))/(np.amax(elem2)-np.amin(elem2))
print(x_train.shape)
for i in range(len(x_valid)):
    elem2 = x_valid[i]-convolve2d(x_valid[i].reshape(28,28),np.array([[1,2,1],[2,4,2],[1,2,1]])/16, mode='same', boundary='fill', fillvalue=0)
    x_valid[i] = (elem2-np.amin(elem2))/(np.amax(elem2)-np.amin(elem2))
for i in range(len(x_test)):
    elem2 = x_test[i]-convolve2d(x_test[i].reshape(28,28),np.array([[1,2,1],[2,4,2],[1,2,1]])/16, mode='same', boundary='fill', fillvalue=0)
