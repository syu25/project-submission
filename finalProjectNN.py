...
def loadData(filename = 'creditcard.csv'):
    data = pd.read_csv(filename)            #load dataset file as pandas dataframe
...
def run_NN():
    ...
    encodingDimension = 14

    modelInput = tf.keras.layers.Input(shape = (inputDimension, ))
    encoding = tf.keras.layers.Dense(encodingDimension, activation = "tanh", activity_regularizer = tf.keras.regularizers.l2(10e-5))(modelInput)
    encoding = tf.keras.layers.Dense(int(encodingDimension / 2), activation = "relu")(encoding)
    decoding = tf.keras.layers.Dense(int(encodingDimension / 2), activation = "tanh")(encoding)
    decoding = tf.keras.layers.Dense(1, activation = "relu")(decoding)
    model = tf.keras.Model(inputs = modelInput, outputs = decoding)
    model.compile(optimizer = tf.keras.optimizers.Adam(learning_rate = learningRate), loss = "binary_crossentropy", metrics = ['accuracy'])
