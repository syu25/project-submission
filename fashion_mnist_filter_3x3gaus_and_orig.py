...
x_train_cpy = np.empty((x_train.shape[0],1568))
x_valid_cpy = np.empty((x_valid.shape[0],1568))
x_test_cpy = np.empty((x_test.shape[0],1568))
...
for i,elem in enumerate(x_train):
    ...
    x_train_cpy[i] = np.append(elem,elem2/np.amax(elem2))
print(x_train.shape)
for i,elem in enumerate(x_valid):
    elem2 = elem-convolve2d(elem.reshape(28,28),np.array([[1,2,1],[2,4,2],[1,2,1]])/16, mode='same', boundary='fill', fillvalue=0)
    x_valid_cpy[i] = np.append(elem,elem2/np.amax(elem2))
for i,elem in enumerate(x_test):
    elem2 = elem-convolve2d(elem.reshape(28,28),np.array([[1,2,1],[2,4,2],[1,2,1]])/16, mode='same', boundary='fill', fillvalue=0)
    x_test_cpy[i] = np.append(elem,elem2/np.amax(elem2))
print("Filters End")

print("Reshape 2 Begin")
x_train = x_train_cpy.reshape(x_train_cpy.shape[0],56,28,1)
x_valid = x_valid_cpy.reshape(x_valid_cpy.shape[0],56,28,1)
x_test = x_test_cpy.reshape(x_test_cpy.shape[0],56,28,1)
print("Reshape 2 End")
...
#model.add(tf.keras.layers.MaxPooling2D(pool_size=2))
model.add(tf.keras.layers.Dropout(.5, input_shape=(56,28,1)))
