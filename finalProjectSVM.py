...
def run_SVM():
    ...
    model.add(tf.keras.layers.Dense(1, activity_regularizer = tf.keras.regularizers.l2(0.01)))
    model.add(tf.keras.layers.Activation('linear'))
    model.compile(loss = 'hinge', optimizer = tf.keras.optimizers.Adagrad(learning_rate = learningRate), metrics = ['accuracy'])
    ...
    testingX, testingY = getTargetValues(testing)
    ...
    plt.scatter(epochs, hist.history['val_loss'], s = 10, marker = "s", label = 'Validation data', c = 'b')
    plt.scatter(epochs, hist.history['loss'], s = 10, marker = "o", label = 'Training data', c = 'r')
